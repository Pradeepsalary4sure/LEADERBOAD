...
// const MAX_DISBURSE_DATE = "2040-12-31";
...
const applyDateRange = () => {
  ...
  const downloadFullReport = () => {
    ...
    reportData.push(["TMinutes Loan - FULL REPORT"]);
    ...
    link.setAttribute('download', `Minutes Loan_Full_Report_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const downloadIncentiveReport = () => {
    ...
    let fileName = `Minutes Loan_Incentive_Report_${new Date().toISOString().split('T')[0]}`;
  ...
  //   name,
  //   email: `${name.toLowerCase().replace(/\s/g, '')}@Minutes Loanloan.com`
  // }));
  ...
  return (
    <div className="app">
      ...
      <div className="split-layout">
        ...
        <main className="main-panel">
          <div className="top-section">
            {showFresh && renderTop3(filteredFresh, "FRESH CASE", "fresh")}
            {showRepeat && renderTop3(filteredRepeat, "REPEAT CASE", "repeat")}
          </div>
